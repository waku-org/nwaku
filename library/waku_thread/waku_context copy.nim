...
type WakuContext* = object
  ...
  # watchdogTouched: Atomic[bool]
  ## used by the watchdog thread to wait certain time until the Waku Thread attends
  ## the watchdog DEBUG request
...
proc watchdogThreadBody(ctx: ptr WakuContext) {.thread.} =
  ...
  let watchdogRun = proc(ctx: ptr WakuContext) {.async.} =
    const WatchdogTimeinterval = 500.milliseconds
    const WakuNotRespondingTimeout = 5.seconds
    while true:
      ...
      ## Initialize the watchdog signal. The Waku Thread will touch it to true
      # ctx.watchdogTouched.store(false)

      # let wakuCallback = proc(
      #     callerRet: cint, msg: ptr cchar, len: csize_t, userData: pointer
      # ) {.cdecl, gcsafe, raises: [].} =
      #   let watchdogTouched = cast[ptr Atomic[bool]](userData)
      #   watchdogTouched[].store(true)

      # let momentReqSent = Moment.now()

      echo "inside watchdogThreadBody: sending watchdog request to Waku thread"
      ...
      # while ctx.watchdogTouched.load == false and numIterations > 0:
      #   await sleepAsync(100.milliseconds)
      #   numIterations.dec()

      # let elapsed = (Moment.now() - momentReqSent).seconds
      # if elapsed > 5:
      #   error "Waku thread is not responding", elapsed
      #   onWakuNotResponding(ctx)
