  ...
  asyncTest "mounting waku rln-relay: check the Merkle tree calculation":
    ...
    asyncTest "mounting waku rln-relay: check dynamic group management":
      ...
      check: 
        ...
        keyPair2.isSome
      let 
        pk1 = keyPair1.get().getIdCommitment() 
        pk2 = keyPair2.get().getIdCommitment() 
      ...
      await node.mountRlnRelay(ethClientAddrOpt = some(EthClient),
                              ethAccAddrOpt = some(ethAccountAddress),
                              memContractAddOpt = some(contractAddress), 
                              memKeyPairOpt = some(keyPair1.get()),
                              memIndexOpt = some(MembershipIndex(0)),
                              pubsubTopic = RLNRELAY_PUBSUB_TOPIC,
                              contentTopic = RLNRELAY_CONTENT_TOPIC,
                              onchainMode = true)
      ...
      await sleepAsync(1000) # wait for the event to reach the group handler
      # pks are inserted into the rln peer's Merkle tree and the resulting root
