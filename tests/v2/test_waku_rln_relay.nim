...
procSuite "Waku rln relay":
  ...
  asyncTest "mounting waku rln relay with static group":
    let
      nodeKey = crypto.PrivateKey.random(Secp256k1, rng[])[]
      node = WakuNode.new(nodeKey, ValidIpAddress.init("0.0.0.0"),
        Port(60000))
    ...
    var groupKeyPairs: seq[MembershipKeyPair]
    var groupIDCommitments: seq[IDCommitment] 
    for i in 0..99:
      groupKeyPairs[i] = MembershipKeyPair(idKey: groupKeys[i].hexToByteArray(32), idCommitment: groupKeys[2*i+1].hexToByteArray(32))
      groupIDCommitments[i] = groupKeys[2*i+1].hexToByteArray(32)
    # create Merkle tree 
    for i in 0..99:
      var member_is_added = false
      member_is_added = rln.insertMember(groupIDCommitments[i])
      doAssert(member_is_added)
      debug "member key", key=groupIDCommitments[i].toHex
    let expectedRoot = rln.getMerkleRoot().value().toHex
    debug "expected root ", expectedRoot

    # start rln-relay
    await node.mountRlnRelay(ethClientAddrOpt = some(EthClient), ethAccAddrOpt =  some(ethAccountAddress), memContractAddOpt =  some(membershipContractAddress), groupOpt = some(groupIDCommitments), memKeyPairOpt = some(groupKeyPairs[index]),  memIndexOpt = some(index))
    let calculatedRoot = node.wakuRlnRelay.rlnInstance.getMerkleRoot().value().toHex
    debug "calculated root ", calculatedRoot
...
suite "Waku rln relay":
  ...
  test "create 100 members":
    ...
    var rln = rlnInstance.value
    # generate a key pair
    var list = newSeq[MembershipKeyPair]()
    var s: string = "["
    for i in 0..99:
      var keypair = rln.membershipKeyGen()
      doAssert(keypair.isSome())
      ...
      echo keypair.get().idKey.toHex
      echo keypair.get().idCommitment.toHex
      
      list.add(keypair.get())
    s = s & "]"
    debug "s", s
