...
proc createSampleList(s: int): seq[IndexedWakuMessage] =
  ...
  for i in 0..<s:
    result.add(IndexedWakuMessage(msg: WakuMessage(payload: @[byte i]), index: Index(timestamp: float64(i), digest: MDigest[256](data: data)) ))
...
procSuite "pagination":
  test "Index computation test":
    ...
    check:
      ...
      index.timestamp == 2 # the timestamp should be a non-zero value
  ...
  test "Index comparison, IndexedWakuMessage comparison, and Sorting tests":
    ...
    let
      index1 = Index(timestamp: 1, digest: MDigest[256](data: data1))
      index2 = Index(timestamp: 1, digest: MDigest[256](data: data2))
      index3 = Index(timestamp: 2, digest: MDigest[256](data: data3))
