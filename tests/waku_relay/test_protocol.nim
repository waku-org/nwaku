  ...
  suite "Send & Retrieve Messages":
    ...
    asyncTest "Valid Payload Sizes":
      ...
      let
        ...
        msg4 = fakeWakuMessage(contentTopic=contentTopic, payload=getByteSequence(1023*1024 + 933)) # 1MiB - 1B -> Max Size (Inclusive Limit)
        msg5 = fakeWakuMessage(contentTopic=contentTopic, payload=getByteSequence(1023*1024 + 934)) # 1MiB -> Max Size (Exclusive Limit)
