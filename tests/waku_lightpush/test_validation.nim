...
suite "WakuLightPush Protocol Tests":
  ...
  test "Validate message size exceeds limit":
    asyncCheck:
      let
        ...
        payload = newSeqWith(DefaultMaxWakuMessageSize + 1, 'a') # Payload that is too large
        msg = WakuMessage(payload: payload.toBytes())
