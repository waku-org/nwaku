...
proc generateStoreA(): Future[ArchiveDriver] {.async.} =
  let
    store = newSqliteArchiveDriver()
    msgList =
      @[
        fakeWakuMessage(
...
proc generateStoreB(): Future[ArchiveDriver] {.async.} =
  let
    store = newSqliteArchiveDriver()
    msgList =
      @[
...
#     let
#       storeA = await generateStoreA()
#       storeB = await generateStoreB()
#       serverA = await newTestWakuStoreNode(serverSwitchA, store = storeA)
#       serverB = await newTestWakuStore(serverSwitchB, store = storeB)
