...
For the ease of demonstration, we make use of Nim-chat, Go-chat and JS-chat applications that are developed on top of their respective Waku v2 clients.
You will need to setup one instance of each chat client in spam-protected mode and then start messaging with them. 
As for the setup, please follow along the tutorials below:
...
Note: JS-chat will use a [WAKU2-LIGHTPUSH protocol](https://rfc.vac.dev/spec/19/) to push its messages to the Waku v2 test fleets. 
...
That is, they do not enforce spam-protection in that specific protocol rather act merely as a message publisher (this behavior may change in the future though).
As such, you should expect receiving spam messages published by the js-chat clients from other connecting chat clients i.e,. Go-chat and Nim-chat.
