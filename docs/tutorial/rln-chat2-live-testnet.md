...
This document is a tutorial on how to run chat2 in spam-protected mode using the waku-RLN-Relay protocol.
In the provided test scenario, you will set up 2 chat2 clients, namely `client1` and `client2`. 
For ease of explanation, we will refer to them as `client1`, `client2`.
`client1` and `client2` are directly connected to waku2 test fleets therefore their messages will be routed via waku2 test fleets.
In this setting, you will try to spam the network using  `client1`  by violating the message rate limit i.e.,
...
Your messages will be routed via test fleets and will arrive at your second `client2`.
Your samp activity will be detected by `client2` and a proper message will be shown on its console.  
If `client2` were to be connected to some other nodes, then it would not route the spam messages further. 

Note that you can also detect the spam activity of other chat2 clients rather than those running by you. 
...
## Create two chat2 nodes
Next, set up the following three chat2 nodes in order.

**Client1 setup**:
Run the following command to set up the first chat2 client. 
In this command, the `rln-relay` flag is set to true to enable RLN-Relay protocol for the spam protection.
...
```
./build/chat2 --content-topic:/toy-chat/2/luzhou/proto --ports-shift=1 --fleet:test --rln-relay:true --rln-relay-membership-index:your_first_index

```

Next, choose your nickname:
```
Choose a nickname >> your_first_nick_name
```
...
```
./build/chat2 --content-topic:/toy-chat/2/luzhou/proto --ports-shift=2 --fleet:test --rln-relay:true --rln-relay-membership-index:your_second_index 

Choose a nickname >> your_second_nick_name
```
...
If two messages sent by the same chat2 client happen to have the same RLN epoch value, then one of them will be detected as spam by all the rest of the network.
At the time of this tutorial, the epoch duration is set to `10` seconds.
