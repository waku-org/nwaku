...
This document is a tutorial on how to run chat2 application over a spam-protected content topic  i.e., `/toy-chat/2/luzhou/proto` using the WAKU-RLN-Relay protocol in the onchain mode.
Unlike prior released tutorials, the group management in the onchain mode is dynamic and is moderated via a membership smart contract that is deployed on the Ethereum testnet of Goerli.
The onchain group management gives the benefit of dynamic group size where the size can grow up to 2^20 members.
...
This transaction will consume some fund from the supplied Ethereum Goerli account. 
Once transaction is mined and the registration is successful, the registered credentials will get displayed on the console.
...
If you choose to not to reuse the same credentials, then for each execution, a new registration takes place and more funds gets deducted from your Ethereum account.
...
- the `--rln-relay-dynamic` flag is set to run the rln-relay protocol in onchain mode with dynamic group management.
...
- the `eth-account-privatekey` option is for your account private key on Goerli testnet. 
  You need to replace `your_eth_private_key` with the private key of your account on Goerli testnet. 
...
Once finalized, the registered  rln identity key, the rln identity commitment key and the index of the registered credentials will be displayed:

```
your membership index is: 63
your rln identity key is: d3a96d33e47234da6bdec4dde4ebcf76a1beb1da451036058105c1354c47972e
...
You can check this by running a second chat user and verifying that spam messages are filtered at the test fleets and wont be displayed. 
...
The two messages `message2` and `message3` have identical RLN epoch value , so, one of them will be discarded by the test fleets as a spam message. 
...
your rln identity key is: dc308718989380f9b76823351fe8ea05c34898178bb9d2400b086a590e37621b
...
your rln identity key is: 0cd504754e3cf113693aa8a84af8fb2b99f48ba6347710024b1686db54cb1b2b
