...
proc init*(T: type App, conf: WakuNodeConf): Result[App, string] =
  ...
  let
    dynamicBootstrapNodesRes =
      retrieveDynamicBootstrapNodes(
        confCopy.dnsDiscovery,
...
proc getRunningNetConfig(app: App): AppResult[NetConfig] =
  ...
  let
    (tcpPort, websocketPort) =
      getPorts(app.node.switch.peerInfo.listenAddrs).valueOr:
  ...
  let
    netConf =
      networkConfiguration(conf, clientId).valueOr:
...
proc updateEnr(app: var App, netConf: NetConfig): AppResult[void] =
  let
    record =
      enrConfiguration(app.conf, netConf, app.key).valueOr:
...
proc updateApp(app: var App): AppResult[void] =
  if app.conf.tcpPort == Port(0) or app.conf.websocketPort == Port(0):
    let
      netConf =
        getRunningNetConfig(app).valueOr:
...
proc startRestServer(
  ...
  let
    requestErrorHandler: RestRequestErrorHandler =
      proc(error: RestRequestError, request: HttpRequestRef): Future[HttpResponseRef] {.
      ...
      .} =
          try:
            ...
            of RestRequestError.InvalidContentBody:
              return await request.respond(
                  Http400, "Invalid content body", HttpTable.init()
                )
            of RestRequestError.InvalidContentType:
              return await request.respond(
                  Http400, "Invalid content type", HttpTable.init()
                )
