...
method startGroupSync*(
  ...
  raise newException(
    CatchableError, "OffchainGroupManager's startGroupSync is not implemented"
  )
...
method register*(
  ...
  raise
    newException(CatchableError, "OffchainGroupManager's register is not implemented")

method registerBatch*(
    g: OffchainGroupManager, rateCommitments: seq[RawRateCommitment]
): Future[void] {.async.} =
  raise newException(
    CatchableError, "OffchainGroupManager's registerBatch is not implemented"
  )
...
method withdraw*(
  ...
  raise
    newException(CatchableError, "OffchainGroupManager's withdraw is not implemented")

method withdrawBatch*(
    g: OffchainGroupManager, idSecretHashes: seq[IdentitySecretHash]
): Future[void] {.async.} =
  raise newException(
    CatchableError, "OffchainGroupManager's withdrawBatch is not implemented"
  )

method onRegister*(g: OffchainGroupManager, cb: OnRegisterCallback) {.gcsafe.} =
  raise
    newException(CatchableError, "OffchainGroupManager's onRegister is not implemented")

method onWithdraw*(g: OffchainGroupManager, cb: OnWithdrawCallback) {.gcsafe.} =
  raise
    newException(CatchableError, "OffchainGroupManager's onWithdraw is not implemented")
