...
proc mountRelay*(node: WakuNode,
  ...
  when defined(rln):
    if rlnRelayEnabled:
      ...
      # if rlnRelayMemIndex < uint32(0) or rlnRelayMemIndex >= uint32(groupSize):
      ...
      #   if expectedRoot != "823d1bb20d78cb748a6e0fd0833623ae8fcaafb284634b8a413b9eb154b71b25":
      #     info "could not create a static group, failed to mount WakuRLNRelay"
      #   else:
      #     waitFor mountRlnRelay(node, groupOpt= some(groupIDCommitment), memKeyPairOpt = some(groupKeyPairs[rlnRelayMemIndex]), memIndexOpt= some(rlnRelayMemIndex))
      #     info "WakuRLNRelay is mounted successfully"
