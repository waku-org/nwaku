...
proc new*(T: type PostgresDriver, conf: WakuNodeConf): ArchiveDriverResult[T] =
  var host : string
  var user : string
  var password : string
  var dbName : string
  ...
  try:
    ...
    let matches = find(conf.storeMessageDbUrl,regex).get.captures
    user = matches[0]
    password =  matches[1]
    host = matches[2]
    port = matches[3]
...
method reset*(s: PostgresDriver) : ArchiveDriverResult[void] =
  try:
...
method init*(s: PostgresDriver) : ArchiveDriverResult[void] =
  try:
    ...
    let
      ...
      exceptionMessage = "failed to init driver, got exception " & repr(e) & " with message " & msg
    return err(exceptionMessage)
  ok()

method put*(s: PostgresDriver, pubsubTopic: PubsubTopic, message: WakuMessage, digest: MessageDigest, receivedTime: Timestamp): ArchiveDriverResult[void] =
  try:
    let res = s.connection.tryExec(s.preparedInsert, toHex(digest.data), receivedTime, message.contentTopic, toHex(message.payload), pubsubTopic, int64(message.version), message.timestamp)
    if not res:
...
proc extractRow(r: Row) : ArchiveDriverResult[ArchiveRow] =
    ...
    ok((pubSubTopic, wakuMessage, @(digest.toOpenArrayByte(0, digest.high)), storedAt))


method getAllMessages*(s: PostgresDriver):  ArchiveDriverResult[seq[ArchiveRow]] =
  ## Retrieve all messages from the store.
  var rows : seq[Row]
  var results : seq[ArchiveRow]
  try:
    rows = s.connection.getAllRows(sql("""SELECT storedAt, contentTopic, payload, pubsubTopic, version, timestamp, id FROM messages ORDER BY storedAt ASC"""))
  except DbError:
...
method getMessagesCount*(s: PostgresDriver): ArchiveDriverResult[int64] =
  var count : int64
  try:
