...
import ../common/nimchronos, ../waku_core, ./rpc, ../common/shard_monitor
...
proc respond(
  ...
  let response = WakuMetadataResponse(
    ...
    shards: m.shardMonitor.getSubscribedShardsUint32(),
  )
...
proc request*(
  ...
  let request = WakuMetadataRequest(
    clusterId: some(m.clusterId), shards: m.shardMonitor.getSubscribedShardsUint32()
  )
...
proc initProtocolHandler(m: WakuMetadata) =
  proc handler(conn: Connection, proto: string) {.async: (raises: [CancelledError]).} =
    ...
    debug "Received WakuMetadata request",
      ...
      localShards = m.shardMonitor.getSubscribedShards(),
      peer = conn.peerId
...
proc new*(T: type WakuMetadata, clusterId: uint32, shardMonitor: ShardMonitor): T =
  let wm = WakuMetadata(clusterId: clusterId, shardMonitor: shardMonitor)

  wm.initProtocolHandler()

  info "Created WakuMetadata protocol",
    clusterId = wm.clusterId, shards = wm.shardMonitor.getSubscribedShards()
