...
proc new*(
  ...
  let shardMonitor = ShardMonitor.new()
  peerManager.setShardGetter(shardMonitor.getShardsGetter())
  let node = WakuNode(
    peerManager: peerManager,
    switch: switch,
    rng: rng,
...
proc mountMetadata*(
  ...
  let metadata = WakuMetadata.new(clusterId, node.shardMonitor)
...
proc subscribe*(
  ...
  node.shardMonitor.onTopicSubscribed(pubsubTopic)
...
proc unsubscribe*(
  ...
  node.shardMonitor.onTopicUnSubscribed(pubsubTopic)
  return ok()
...
proc filterSubscribe*(
  ...
  if pubsubTopic.isSome():
    ...
    if subRes.isOk():
      ...
      node.shardMonitor.onTopicSubscribed(pubsubTopic.get())
    else:
    ...
    try:
      ...
      for pubsub, topics in topicMap.pairs:
        ...
        node.shardMonitor.onShardSubscribed(pubsub.shardId.uint16)
    except CatchableError:
...
proc filterUnsubscribe*(
  ...
  if pubsubTopic.isSome():
    ...
    if unsubRes.isOk():
      ...
      node.shardMonitor.onTopicUnSubscribed(pubsubTopic.get())
    else:
    ...
    try:
      ...
      for pubsub, topics in topicMap.pairs:
        ...
        node.shardMonitor.onShardUnSubscribed(pubsub.shardId.uint16)
    except CatchableError:
