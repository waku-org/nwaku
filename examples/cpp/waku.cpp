...
typedef void (*CallbackFunc) (char* msg, size_t len_0);
...
int main(int argc, char** argv) {
    ...
    WAKU_CALL(waku_new(jsonConfig, [](char* msg, size_t len) {
    ...
    static std::string pubsubTopic;
    WAKU_CALL(waku_pubsub_topic((char*) "example", cify([&](char* msg, size_t len) {
        pubsubTopic = msg;
    })));

    std::cout << "Jamon [" << pubsubTopic << "]" << std::endl;
